...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	kvadSpeed := math.Pow(meanSpeed(action, duration)*kmhInMsec, 2)
	return ((walkingCaloriesWeightMultiplier*weight + (kvadSpeed/height)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
